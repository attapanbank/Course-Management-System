...
<body>
	...
	<%
		...
		out.println("<div align=" + "center" + "><font size=" + 6 + ">"
				+ coursecode + " " + coursename + "<br>" + "Lect : " + lect
				+ " Lab : " + lab + "</font></div>");
	%>
	<br>
	<br>
		...
		<tbody>
			<%
				...
				String Querysection = "SELECT * FROM section INNER JOIN candidate ON candidate.sectionID = section.sectionID INNER JOIN user ON candidate.userID = user.userID INNER JOIN currentcourse ON currentcourse.currentcourseID = section.currentcourseID WHERE currentcourse.year = '"
						...
						+ sectionid
						+ "' GROUP BY user.userID ORDER BY candidate.teachtype desc";
				ResultSet rssection = stmt.executeQuery(Querysection);
				while (rssection.next()) {
					stmt = con.createStatement();
					String QueryCandidate = "SELECT * FROM section INNER JOIN candidate ON candidate.sectionID = section.sectionID INNER JOIN user ON candidate.userID = user.userID INNER JOIN currentcourse ON currentcourse.currentcourseID = section.currentcourseID WHERE currentcourse.year = '"
							+ year
							+ "' AND currentcourse.semester = '"
							+ term
							+ "' AND section.sectionID = '"
							+ sectionid
							+ "' AND user.userID = '"
							+ rssection.getString("user.userID") + "' ORDER BY candidate.teachtype desc";
				...
				<td>
					<%
						while (rscandidate.next()) {
								out.println("- "
										+ rscandidate.getString("candidate.teachtype")
										+ "<br>");
							}
					%>
				</td>
